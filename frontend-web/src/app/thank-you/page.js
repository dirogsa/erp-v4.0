...
export default function GraciasPage() {
  ...
  const [clientInfo, setClientInfo] = useState({ name: '', company: '', isSynced: false });
  ...
  useEffect(() => {
    ...
    const clientData = clientDataStr ? JSON.parse(clientDataStr) : { name: 'Cliente B2B', company: '', is_erp_synced: false };
    setClientInfo({ name: clientData.name, company: clientData.company, isSynced: clientData.is_erp_synced });
    ...
    let waText = clientData.is_erp_synced 
      ? `*PEDIDO B2B REGISTRADO EN ERP* ✅\n`
      : `*NUEVA COTIZACIÓN B2B* 📦\n`;
      
    waText += `-------------------\n`;
    waText += `👤 Nombre: ${clientData.name}\n`;
    if (clientData.company) waText += `🏢 Empresa/RUC: ${clientData.company}\n`;
    if (clientData.is_erp_synced) waText += `⚡ Estado: Inyectado a ERP\n`;
    waText += `-------------------\n`;
    ...
    if (clientData.is_erp_synced) {
      waText += `Hola, acabo de confirmar este pedido desde mi cuenta B2B. Quedo atento al despacho.`;
    } else {
    ...
    <div className="min-h-[80vh] flex items-center justify-center p-4">
      <div className="relative rounded-[2.5rem] p-[2px] overflow-hidden group max-w-lg w-full text-center">
        ...
        <div className="relative rounded-[2.5rem] p-10 flex flex-col items-center justify-center space-y-6 backdrop-blur-xl bg-[#0A0A0B]/90 border border-white/5">
          ...
          <div>
            <h1 className="text-white font-black text-3xl uppercase tracking-tighter mb-2">
              {clientInfo.isSynced ? '¡Pedido Registrado!' : '¡Cotización Lista!'}
            </h1>
            <p className="text-sm leading-relaxed text-white/60 mb-6">
              {clientInfo.isSynced 
                ? 'Tu pedido ha sido inyectado directamente a nuestro sistema ERP. Haz clic abajo para notificar a tu asesor de ventas.' 
                : 'Tu lista de repuestos ha sido procesada. Haz clic en el botón abajo para enviarnos tu pedido por WhatsApp y un asesor te responderá con disponibilidad y precios.'}
            </p>
          </div>
          ...
          {waLink ? (
            ...
            >
              ...
              {clientInfo.isSynced ? 'Notificar al Asesor' : 'Enviar a WhatsApp'}
            </a>
          ) : (
