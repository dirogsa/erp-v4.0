...
async def process_b2b_application(
    ...
    application.processed_by = current_user.email
    await application.save()

    if process_in.status == B2BStatus.APPROVED:
        # Check if user already exists
        user = await User.find_one(User.email == application.email)
        if user:
            user.role = UserRole.CUSTOMER_B2B
            user.ruc_linked = application.ruc
            await user.save()
        else:
            # Create user with a generated password? 
            # For now, if we approved, we assume they should register or we promote the existing one.
            # Usually, the user is already registered as B2C and then applies.
            pass
