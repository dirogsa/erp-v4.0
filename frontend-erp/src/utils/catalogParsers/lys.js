 ...
 * Utiliza el formato HTML de Millardcatalog.com como plantilla, 
 * ya que LYS no cuenta con una página web estructurada para extracción directa.
 */
export const parseLys = (doc, domain = 'http://www.millardcatalog.com', dbCategories = []) => {
    const data = {
        brand: 'LYS',
        sku: '',
        name: '',
    ...
    if (searchAlert && searchAlert.textContent.includes('searching for')) {
        ...
        if (searchedMatch && searchedMatch[1]) {
            ...
            if (eqCode && eqCode !== data.sku) {
                // Inferir marca rudimentariamente
                let eqBrand = 'UNKNOWN';
                if (eqCode.startsWith('ML') || eqCode.startsWith('MZ')) eqBrand = 'MILLARD';
                else if (eqCode.startsWith('W')) eqBrand = 'WIX';
                else if (eqCode.startsWith('C') || eqCode.startsWith('PH')) eqBrand = 'FRAM';
