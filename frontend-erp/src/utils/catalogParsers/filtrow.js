...
export const parseFiltrow = (doc, baseUrl = 'https://www.jsfilter.jp', dbCategories = []) => {
    try {
        ...
        if (titleElement) {
            ...
            // El HTML usa &raquo; que DOMParser decodifica a » (U+00BB)
        ...
        // 2. Imagen (Motor de extracción prioritaria HD)
        const anchorElement = doc.querySelector('#achrColorBox');
        const imgElement = doc.querySelector('#achrColorBox img');
        
        let src = null;
        if (anchorElement && anchorElement.getAttribute('href')) {
            src = anchorElement.getAttribute('href'); // 🥇 Prioridad 1: Imagen HD (watermark)
        } else if (imgElement) {
            src = imgElement.getAttribute('src'); // 🥈 Fallback: Miniatura (thumb)
        }
        ...
        specRows.forEach(row => {
            const label = row.querySelector('.param-title')?.textContent.trim();
            const value = row.querySelector('.param-field')?.textContent.trim();
            if (label && value) {
        ...
        refRows.forEach(row => {
            const brand = row.querySelector('.owner')?.textContent.trim();
            const code = row.querySelector('.field')?.textContent.trim();
            if (brand && code) {
        ...
        modelTitles.forEach(titleDiv => {
            ...
            if (tableContainer && tableContainer.classList.contains('model-body')) {
                ...
                rows.forEach(row => {
                    const year = row.querySelector('.tdYear')?.textContent.trim();
                    const engine = row.querySelector('.tdEngineVol')?.textContent.trim();
                    const engineCode = row.querySelector('.tdEngineNo')?.textContent.trim();
                    const body = row.querySelector('.tdBody')?.textContent.trim();
