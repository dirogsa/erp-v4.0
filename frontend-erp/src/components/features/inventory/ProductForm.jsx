...
const ProductForm = ({
    ...
    // Auto-generate SKU for new marketing products
    useEffect(() => {
        if (!isEditMode && !isViewMode && formData.type === 'MARKETING' && !formData.sku) {
            handleGenerateSku();
        }
    }, [formData.type]);
    ...
    const handleGenerateSku = async () => {
        try {
            const res = await inventoryService.generateMarketingSku();
    ...
    return (
        <form onSubmit={handleSubmit} style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div style={{ display: 'grid', gap: '1rem' }}>
                    ...
                    {!fixedType && (
                        <Select
                            ...
                            options={[
                                { value: 'COMMERCIAL', label: '📦 Producto Comercial (Filtros, Repuestos)' },
                                { value: 'MARKETING', label: '🎁 Artículo Publicitario / Premios (Polos, Shorts, etc.)' }
                            ]}
                            value={formData.type}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: formData.type === 'COMMERCIAL' ? '1fr 1fr' : '1fr', gap: '1rem' }}>
                        {formData.type === 'COMMERCIAL' && (
                            <Input
                                label="Puntos Ganados (Venta)"
                                type="number"
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                        <label style={{ color: '#94a3b8', fontSize: '0.875rem', fontWeight: '500' }}>Descripción</label>
                        <textarea
                            style={{
                                width: '100%',
                            ...
                            placeholder="Ej: Polo de algodón 100% con logo bordado..."
                            disabled={isViewMode}
                        />
                    </div>
            ...
            {/* --- Secciones Técnicas (Solo para Productos Comerciales) --- */}
            {formData.type === 'COMMERCIAL' && (
                <>
                    ...
                    {selectedCategory && (
                        <>
                            ...
                            {selectedCategory.attributes_schema?.length > 0 && (
                                <div style={sectionStyle}>
                                    <h3 style={sectionTitleStyle}>Atributos Específicos</h3>
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
                                        {selectedCategory.attributes_schema.map((attr) => (
                                            <div key={attr.key}>
                                                {attr.type === 'select' ? (
                                                    <Select
                                                        label={attr.label}
